...
public interface EmployeeRepository extends JpaRepository <Employee, Long> {
    ...
    @Query(value = "SELECT * FROM employees WHERE  super_visor_id LIKE %:superVisor%",
            nativeQuery =
            true)

List<Employee> search(@Param("superVisor") String superVisor);

    @Query(value = "SELECT * FROM employees WHERE employee_type_enum LIKE %:employeeTypeEnum% ",nativeQuery = true)
